...
namespace _102190156_TranKimTienDat
{
    class QLKH_DAL
    {
        ...
        public List<SanPham> getAllSanPham_DAL(string ten, string ncc, string matp)
        {
            List<SanPham> sps = new List<SanPham>();
            string query = "SELECT * FROM SanPhams INNER JOIN NCCs ON SanPhams.MaNCC = NCCs.MaNCC " 
                + "WHERE NCCs.MaNCC = '" + ncc +  "' AND NCCs.MaTP IN (SELECT MaTP FROM DCs WHERE MaTP = '" + matp + "')";
            if (matp == "")
            {
            ...
            else
            {
                if (ncc == "")
                {
                    query = "SELECT * FROM SanPhams INNER JOIN NCCs ON SanPhams.MaNCC = NCCs.MaNCC "
                        + "WHERE NCCs.MaTP IN (SELECT MaTP FROM DCs WHERE MaTP = '" + matp + "')";
                    foreach (DataRow dr in DBHelper.Instance.getRecord(query).Rows)
                    {
                        if (ten != "")
                        {
        ...
        public NCC getNCCbyMaNCC_DAL(int ma)
        {
        ...
        public List<NCC> getNCCbyMaTP_DAL(string ma)
        {
            List<NCC> nccs = new List<NCC>();
            string query = "SELECT * FROM NCCs WHERE MaTP = '" + ma + "'";
            foreach (DataRow dr in DBHelper.Instance.getRecord(query).Rows)
            {
                nccs.Add(get1NCC(dr));
            }
        ...
        public DC getTPbyMaNCC_DAL(int mancc)
        {
            string query = "SELECT * FROM DCs INNER JOIN NCCs ON DCs.MaTP = NCCs.MaTP WHERE NCCs.MaNCC = '" + mancc.ToString() + "'";
            return get1TP(DBHelper.Instance.getRecord(query).Rows[0]);
        }
    }
}
